...
loss_op2 = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=logits_test, labels=Y2))
optimizerr = tf.train.AdamOptimizer(learning_rate=learning_rate)
test_op2 = optimizerr.minimize(loss_op2)
...
with tf.Session() as sess:
    ...
    for step in range(1, num_steps+1):
        ...
        if step % display_step == 0:
            # Run optimization and calculate batch loss and accuracy
            _, loss2, acc2 = sess.run([test_op2, loss_op2, accuracy])
            print("Step " + str(step) + ", Minibatch Loss= " + \
                  "{:.4f}".format(loss2) + ", Testing Accuracy= " + \
                  "{:.3f}".format(acc2*100))
        else:
            # Only run the optimization op (backprop)
            sess.run(test_op2)        
